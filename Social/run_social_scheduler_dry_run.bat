...
echo [INFO] Running single-process DRY-RUN with both targets files
set "SOCIAL_SCHEDULER_SUMMARY_FILE=%TEMP%\social_scheduler_dry_run_summary_%RANDOM%_%RANDOM%.txt"
if exist "%SOCIAL_SCHEDULER_SUMMARY_FILE%" del "%SOCIAL_SCHEDULER_SUMMARY_FILE%" >nul 2>&1
%PYTHON_EXE% social_scheduler.py --dry-run --accounts-dir config --targets config/targets.json --targets config/targets_admin.json --posts config/posts.json --performance-mode --concurrency 2
set "OVERALL_EXIT=%ERRORLEVEL%"

echo.
echo ===== Scheduler DRY-RUN Summary =====
